 ...
 */
public class CustomDataSetExecutorImpl implements DataSetExecutor {

    public static final String DEFAULT_EXECUTOR_ID = "default";

    private static final Logger log = LoggerFactory.getLogger(CustomDataSetExecutorImpl.class);

    private static final Map<String, CustomDataSetExecutorImpl> executors = new ConcurrentHashMap<>();
    ...
    public static CustomDataSetExecutorImpl instance(ConnectionHolder connectionHolder) {
        // if no executor name is provided use default
        return instance(DEFAULT_EXECUTOR_ID, connectionHolder);
    }

    public static CustomDataSetExecutorImpl instance(String executorId, ConnectionHolder connectionHolder) {
        CustomDataSetExecutorImpl instance = executors.get(executorId);
        if (instance == null) {
            instance = new CustomDataSetExecutorImpl(executorId, connectionHolder,
    ...
    private CustomDataSetExecutorImpl(String executorId, ConnectionHolder connectionHolder, DBUnitConfig dbUnitConfig) {
            ...
            try {
                ...
                if (dataSetConfig.isCleanBefore()) {
                    try {
                        ...
                        LoggerFactory.getLogger(CustomDataSetExecutorImpl.class.getName())
    ...
    private void logDataSet(IDataSet resultingDataSet, Exception e) {
        try {
            ...
            try (FileOutputStream fos = new FileOutputStream(datasetFile)) {
                XmlDataSet.write(resultingDataSet, fos);
            }
        } catch (Exception e1) {
    ...
    private DatabaseOperation getOperation(DataSetConfig dataSetConfig) {
        ...
        if (getRiderDataSource().getDBType() == DBType.MSSQL && dataSetConfig.isFillIdentityColumns()) {
            switch (strategy) {
                case INSERT:
    ...
    public IDataSet loadDataSet(String name) throws DataSetException, IOException {
        ...
        for (String dataSet : dataSetNames) {
            ...
            switch (extension) {
                ...
                case "xml": {
                    try {
                        target = new ScriptableDataSet(sensitiveTableNames, new XmlDataSetBuilder()
                                .build(getDataSetUrl(dataSetName).openStream()));
                    } catch (Exception e) {
                        target = new ScriptableDataSet(sensitiveTableNames, new XmlDataSetBuilder()
    ...
    public boolean equals(Object other) {
        if (!(other instanceof CustomDataSetExecutorImpl)) {
            return false;
        }
        CustomDataSetExecutorImpl otherExecutor = (CustomDataSetExecutorImpl) other;
    ...
    public static CustomDataSetExecutorImpl getExecutorById(String id) {
