...
public class AssignServiceImpl implements AssignService {
    ...
    @Autowired
    public AssignServiceImpl(RoleRepository roleRepository, PermissionRepository permissionRepository, RegularUserRepository userRepository) {
    ...
    public void assignRolePermission(Long roleId, Long permissionId) throws ResourceNotFoundException {
        ...
        final var role = getRole(roleId);
        final var permission = getPermission(permissionId);
    ...
    private PermissionEntity getPermission(Long id) throws ResourceNotFoundException {
        if (id == null)
            throw new ResourceNotFoundException(format(VALIDATION_NOT_FOUND_MSG, PERMISSION, "null"));
        LOGGER.debug(format("finding permission with id {0}"), id);
        var permission = permissionRepository.findById(id);
        if (permission.isEmpty())
            throw new ResourceNotFoundException(format(VALIDATION_NOT_FOUND_MSG, PERMISSION, id));
    ...
    private RoleEntity getRole(Long id) throws ResourceNotFoundException {
        if (id == null)
            throw new ResourceNotFoundException(format(VALIDATION_NOT_FOUND_MSG, ROLE, "null"));
        LOGGER.debug(format("finding role with id {0}"), id);
        var role = roleRepository.findById(id);
        if (role.isEmpty())
            throw new ResourceNotFoundException(format(VALIDATION_NOT_FOUND_MSG, ROLE, id));
    ...
    private RegularUserEntity getUser(Long id) throws ResourceNotFoundException {
        if (id == null)
            throw new ResourceNotFoundException(format(VALIDATION_NOT_FOUND_MSG, USER, "null"));
        LOGGER.debug(format("finding user with id {0}"), id);
        var user = userRepository.findById(id);
        if (user.isEmpty())
            throw new ResourceNotFoundException(format(VALIDATION_NOT_FOUND_MSG, USER, id));
